...
public class Number_Factors_TopDown {
	...
	private int waysToGetN_TopDown(int[] dp, int n) {
		 if(n == 0 || n == 1 || n == 2) return 1;  // { }, {1}, {1,1}
		 if(n == 3) return 2; // {1,1,1}, {3}
		 if(dp[n] == 0) {
			 int subsruct1 = waysToGetN_TopDown(dp, n-1);;// if we subtract 1, we will be left with 'n-1'
			 ...
			 dp[n] = subsruct1+subtract3+subtract4;
		 }
		return dp[n];
	}// end of method
